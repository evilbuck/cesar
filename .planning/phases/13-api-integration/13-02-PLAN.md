...
2. Add _get_hf_token method to BackgroundWorker:
   - Accept optional CesarConfig in __init__ (store as self.config)
...
3. Update __init__ to accept optional config parameter:
   ...
   - Store config for HF token resolution
...
6. Handle retry case in _process_job:
...
For Phase 13, update progress at phase boundaries only (not real-time).
  ...
  <verify>python -c "from cesar.api.worker import BackgroundWorker; from cesar.api.models import Job; print('Worker imports OK')"</verify>
  <done>Worker uses orchestrator when diarize=True, handles HF token, sets PARTIAL on failures</done>
</task>
...
5. Ensure progress fields are included in repository.update calls.
  ...
  <verify>python -c "from cesar.api.worker import BackgroundWorker; print('Progress tracking imports OK')"</verify>
...
Use existing test patterns from the file (async tests, mocking, in-memory DB).
  ...
  <done>Worker tests cover diarization enabled/disabled, HF token missing, fallback, retry</done>
</task>
