...
Note on DIAR-05: REQUIREMENTS.md states "inline bold labels" but CONTEXT.md (user decisions from discuss-phase) explicitly specifies "### Speaker N" section headers. CONTEXT.md is authoritative - implementing section headers as shown in the format example. Consider updating REQUIREMENTS.md wording after Phase 11 completes.
</objective>
...
Run tests to confirm they fail (RED phase).
  ...
  <verify>python -m pytest tests/test_transcript_formatter.py -v --tb=short 2>&1 | grep -E "(FAILED|PASSED|ERROR)" shows test results (expect failures or import errors before implementation)</verify>
  <done>13+ tests exist in tests/test_transcript_formatter.py covering all formatting scenarios</done>
</task>
