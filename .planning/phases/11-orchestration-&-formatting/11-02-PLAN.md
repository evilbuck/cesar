...
<task type="auto">
  <name>Task 1: Create orchestrator module with result dataclass</name>
...
3. Orchestrate method flow:
   a. Step 1 (0-60%): Transcribe audio
      - Call transcriber.transcribe_file()
      - Update progress: "Transcribing...", 0-60%
      - Parse segments from output file for alignment
   ...
   c. Step 3 (90-100%): Format and save
      - If diarization succeeded:
        - Call align_timestamps() to merge transcription + diarization
        - Call formatter.format_with_speakers()
        - Save as .md file
        - Update progress: "Formatting...", 90-100%
      - If diarization failed:
...
<task type="auto">
  <name>Task 2: Create comprehensive orchestrator tests</name>
...
1. test_orchestrate_success_with_diarization
   - Mock transcriber, diarizer, formatter
   - Verify correct flow: transcribe -> diarize -> align -> format -> save
...
<task type="auto">
  <name>Task 3: Integration verification and commit</name>
  <files>cesar/orchestrator.py, tests/test_orchestrator.py</files>
  <action>
...
2. Orchestrator tests pass: `python -m pytest tests/test_orchestrator.py -v`
3. Full test suite passes: `python -m pytest tests/ -v`
4. Orchestrator correctly coordinates transcriber -> diarizer -> formatter flow
5. Fallback to plain text works when diarization fails
</verification>
